...
private ["_id","_situation","_mission","_description","_sustainment","_ao"];
...
[GVAR(areas),{
    ...
    // setup description
    _situation = format ["...",_value getVariable [QGVAR(name),""]];
    _mission = format ["...",_value getVariable [QGVAR(name),""]];

    _description = [_value,_situation,_mission] call FUNC(taskOPORD);

    // create task
    [true,_id,[_description select 1,_description select 0,""],[_value getVariable [QEGVAR(main,polygon),DEFAULT_POLYGON]] call EFUNC(main,polygonCentroid),"CREATED",-1,false,(_value getVariable [QGVAR(name),""]) select [0,1]] call BIS_fnc_taskCreate;
...
[GVAR(outposts),{
    ...
    _situation = format ["Some time ago, a convoy of military logistics and engineering vehicles was spotted entering AO %1. These vehicles are likely supporting the construction of an enemy outpost (OBJ %2). Previous engagements have shown the enemy to be composed of dismounted patrols and potentially, static emplacements. They are expected to defend upon contact.",_ao getVariable [QGVAR(name),""],_value getVariable [QGVAR(name),""]];
    _mission = format ["Friendly forces will conduct an operation in AO %1 to locate and secure OBJ %2 in order to prevent anti-coalition forces from gaining momentum and swaying civilian approval.",_ao getVariable [QGVAR(name),""], _value getVariable [QGVAR(name),""]];
