...
[GVAR(outposts),{
    ...
    */
    ...
    // @todo some units dont move to position without forcing setPos
    ...
    */
    ...
    _intelObjects = (_composition select 2) select {toLower typeOf _x in INTEL_BASES};
    
    if (PROBABILITY(0.75) && {!(_intelObjects isEqualTo [])}) then {
        _intelBase = selectRandom _intelObjects;
    } else {
        _intelNodes = (_composition select 1) select {(_x select 1) >= 1};
        
        if (_intelNodes isEqualTo []) exitWith {};

        _intelBase = createSimpleObject [selectRandom INTEL_BASES,DEFAULT_SPAWNPOS];
        _intelBase setDir (random 360);
        [_intelBase,ATLtoASL ((selectRandom _intelNodes) select 0)] call EFUNC(main,setPosSafe);
    };

    if !(isNil "_intelBase") then {
        _size = [_intelBase] call EFUNC(main,getObjectSize);

        _intelPos = getPosASL _intelBase;
        ...
        if (GVAR(intelPrimary) isEqualTo []) then {
            GVAR(intelPrimary) pushBack _intel;
        } else {
            GVAR(intelSecondary) pushBack _intel;
        }; 
    } else {
        ERROR_1("%1 does not have a suitable intel node",_composition select 3);
    };
