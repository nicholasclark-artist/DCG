...
call compileFinal preprocessFileLineNumbers "\d\dcg\addons\main\scripts\zbe_cache\zbe_functions.sqf";
	...
	while {true} do {
		...
		zbe_players = (switchableUnits + playableUnits);
		{
			...
			if (!_disable && {!(_x in zbe_cachedGroups)}) then {
				 ...
				 [zbe_aiCacheDist, _x, zbe_minFrameRate, zbe_debug] execFSM "\d\dcg\addons\main\scripts\zbe_cache\zbe_aiCaching.fsm";
				};
		} forEach allGroups;
	};
};
...
[] spawn {
	...
	while {true} do {
		_assetscar = zbe_centerPOS nearEntities ["LandVehicle", zbe_mapside];
		{
			if !(_x in zbe_cached_cars) then {
				...
				zbe_cached_cars pushBack _x;
					[_x, zbe_vehicleCacheDistCar] execFSM "\d\dcg\addons\main\scripts\zbe_cache\zbe_vehicleCaching.fsm";
			};
		} forEach _assetscar;
		_assetsair = zbe_centerPOS nearEntities ["Air", zbe_mapside];
		{
			if !(_x in zbe_cached_air) then {
				...
				zbe_cached_air pushBack _x;
				    [_x, zbe_vehicleCacheDistAir] execFSM "\d\dcg\addons\main\scripts\zbe_cache\zbe_vehicleCaching.fsm";
			};
		} forEach _assetsair;
		_assetsboat = zbe_centerPOS nearEntities ["Ship", zbe_mapside];
		{
			if !(_x in zbe_cached_boat) then {
				...
				zbe_cached_boat pushBack _x;
					[_x, zbe_vehicleCacheDistBoat] execFSM "\d\dcg\addons\main\scripts\zbe_cache\zbe_vehicleCaching.fsm";
			};
		} forEach _assetsboat;

		{
			...
			// zbe_cached_cars = zbe_cached_cars - [_x];
			zbe_cached_cars deleteAt _forEachIndex;
			};
		} forEach zbe_cached_cars;
		{
			if (!(_x in _assetsair)) then {
			// zbe_cached_air = zbe_cached_air - [_x];
			zbe_cached_air deleteAt _forEachIndex;
			};
		} forEach zbe_cached_air;
		{
			if (!(_x in _assetsboat)) then {
			// zbe_cached_boat = zbe_cached_boat - [_x];
			zbe_cached_boat deleteAt _forEachIndex;
			};
...
// if (!isDedicated) then {execFSM "\d\dcg\addons\main\scripts\zbe_cache\zbe_clientObjectDrawAuto.fsm";};